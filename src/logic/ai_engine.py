import os
import streamlit as st
from langchain_google_genai import ChatGoogleGenerativeAI
from langchain_core.prompts import PromptTemplate
from dotenv import load_dotenv

load_dotenv()

def get_api_key():
    key = os.getenv("GOOGLE_API_KEY")
    if not key and "GOOGLE_API_KEY" in st.secrets:
        key = st.secrets["GOOGLE_API_KEY"]
    return key

def get_llm():
    api_key = get_api_key()
    if not api_key:
        raise ValueError("❌ ไม่พบ Google API Key! กรุณาตั้งค่าใน .env หรือ Secrets")
    
    return ChatGoogleGenerativeAI(
        model="gemini-1.5-flash", 
        google_api_key=api_key,
        temperature=0.3
    )

def extract_rules_from_pdf_text(raw_text):
    try:
        llm = get_llm()
        prompt_template = """
        คุณคือผู้เชี่ยวชาญด้านกฎหมายแรงงาน หน้าที่ของคุณคือแปลงเอกสารเป็นกฎระเบียบสำหรับระบบ AI
        
        คำสั่ง:
        1. อ่านข้อความต้นฉบับ
        2. สกัดเงื่อนไขที่เป็น "ข้อห้าม" หรือ "ข้อกำหนด" (Do's & Don'ts)
        3. ตัดน้ำท่วมทุ่งทิ้ง เอาแต่เนื้อๆ
        4. สำคัญมาก: ต้องคั่นระหว่างข้อด้วย "--------------------" (ขีดกลาง 20 ที) เสมอ
        
        ข้อความต้นฉบับ:
        {text}
        
        ผลลัพธ์ (Format: กฎข้อที่ 1 ... \n--------------------\n กฎข้อที่ 2 ...):
        """
        prompt = PromptTemplate(template=prompt_template, input_variables=["text"])
        chain = prompt | llm
        return chain.invoke({"text": raw_text}).content
    except Exception as e:
        return f"Error extracting rules: {e}"

def generate_course_design(job_title, duration, objectives, context, retrieved_rules):
    try:
        llm = get_llm()
        
        design_prompt = """
        คุณคือ "DSD Course Architect" นักออกแบบหลักสูตรกรมพัฒนาฝีมือแรงงาน
        
        ข้อมูลโจทย์:
        - หลักสูตรสำหรับ: {job_title}
        - ระยะเวลา: {duration}
        - วัตถุประสงค์หลัก: {objectives}
        - ข้อมูลเพิ่มเติม: {context}
        
        📚 กฎระเบียบอ้างอิง:
        "{rules}"
        
        คำสั่ง:
        จงออกแบบหลักสูตรฝึกอบรมฉบับสมบูรณ์ (ตอบเป็นภาษาไทยเท่านั้น)
        
        ⏰ กฎเหล็กเรื่องเวลา (Time Constraints):
        1. **1 วัน ต้องฝึกอบรม 6 ชั่วโมงเต็ม** (ไม่รวมพัก)
        2. **ตารางเวลามาตรฐาน** ต้องเป็นดังนี้เท่านั้น (ห้ามเปลี่ยนเวลา):
           - 09:00 - 10:30 (1.5 ชม.)
           - 10:30 - 10:45 (พักเบรกเช้า 15 นาที)
           - 10:45 - 12:15 (1.5 ชม.)
           - 12:15 - 13:15 (พักรับประทานอาหารกลางวัน 1 ชั่วโมง)
           - 13:15 - 14:45 (1.5 ชม.)
           - 14:45 - 15:00 (พักเบรกบ่าย 15 นาที)
           - 15:00 - 16:30 (1.5 ชม.)
        3. ถ้าหลักสูตรมีหลายวัน ให้ทำซ้ำตารางเดิม แต่เปลี่ยนหัวข้อวิชา
        
        ⚠️ กฎเหล็กเรื่องเนื้อหา (Content Rules):
        1. **ต้องระบุ (ทฤษฎี) หรือ (ปฏิบัติ) หน้าชื่อวิชาเสมอ** เช่น "[ทฤษฎี] ความปลอดภัยในการทำงาน", "[ปฏิบัติ] การเดินสายไฟ"
        2. สร้างตาราง Markdown 4 คอลัมน์: เวลา | หัวข้อวิชา (ระบุ ทฤษฎี/ปฏิบัติ) | กิจกรรม/วิธีการฝึก | มาตรฐาน/กฎที่เกี่ยวข้อง
        3. ห้ามรวมบรรทัด (No Merged Cells) ให้เขียนทีละบรรทัดเวลา
        4. ชื่อหลักสูตรต้องเป็นทางการ (ห้ามใช้ภาษาการตลาด)
        
        รูปแบบการตอบ (Response Format) - ใช้ Markdown ตามนี้เป๊ะๆ:
        
        # หลักสูตร: [ชื่อหลักสูตรเป็นทางการ]

        ## 1. หลักการและเหตุผล
        [เนื้อหา...]

        ## 2. วัตถุประสงค์
        * [ข้อ 1]
        * [ข้อ 2]

        ## 3. กลุ่มเป้าหมาย
        * {job_title}

        ## 4. กำหนดการฝึกอบรม (Schedule)
        
        | เวลา | หัวข้อวิชา | กิจกรรม/วิธีการฝึก | มาตรฐาน/กฎที่เกี่ยวข้อง |
        | :--- | :--- | :--- | :--- |
        | 09:00 - 10:30 | **[ทฤษฎี]** [ชื่อวิชา] | [รายละเอียด] | [อ้างอิงกฎ] |
        | 10:30 - 10:45 | **พักรับประทานอาหารว่าง** | - | - |
        | 10:45 - 12:15 | **[ปฏิบัติ]** [ชื่อวิชา] | [รายละเอียด] | [อ้างอิงกฎ] |
        | 12:15 - 13:15 | **พักรับประทานอาหารกลางวัน** | - | - |
        | 13:15 - 14:45 | **[ปฏิบัติ]** [ชื่อวิชา] | [รายละเอียด] | [อ้างอิงกฎ] |
        | 14:45 - 15:00 | **พักรับประทานอาหารว่าง** | - | - |
        | 15:00 - 16:30 | **[ปฏิบัติ]** [ชื่อวิชา] | [รายละเอียด] | [อ้างอิงกฎ] |
        
        ## 5. การประเมินผล
        * [เนื้อหา...]

        ## 6. สรุปความสอดคล้องกับกฎระเบียบ
        * [เนื้อหา...]
        """
        
        prompt = PromptTemplate(template=design_prompt, input_variables=["job_title", "duration", "objectives", "context", "rules"])
        chain = prompt | llm
        return chain.invoke({
            "job_title": job_title, 
            "duration": duration, 
            "objectives": objectives, 
            "context": context, 
            "rules": retrieved_rules
        }).content
    except Exception as e:
        return f"Error designing course: {e}"